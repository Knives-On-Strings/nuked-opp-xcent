...
typedef struct {
    ...
    // OPP attack-rate plateau (DSP23): OPP AR=18-30 produce ~105-115 ms attacks
    // versus OPM AR=13's 95 ms. Native OPM rate granularity is too coarse to hit
    // these intermediate times, so we slow the attack by skipping every Nth EG
    // inc=1 tick during attack state. opp_atk_skip[slot]==0 disables this (default).
    // skip=N effectively makes attack (N+1)/N times longer (e.g. skip=10 → 1.10x).
    uint8_t opp_atk_skip[32];
    uint8_t opp_atk_skip_cnt[32];
 ...
 * time by approximately (N+1)/N (e.g. skip=10 -> 1.10x slower). Used to model
 * the OPP AR=18-30 attack plateau (~105-115 ms) which falls between OPM AR=12
 * (140 ms) and AR=13 (95 ms). */
