...
static int32_t OPP_KCToFNum(int32_t kcode)
{
...
static int32_t OPP_LFOApplyPMS(int32_t lfo, int32_t pms)
{
    int32_t t, out;
    int32_t top = (lfo >> 4) & 7;
    if (pms != 7)
...
static int32_t OPP_CalcKCode(int32_t kcf, int32_t lfo, int32_t lfo_sign, int32_t dt)
{
...
static void OPP_PhaseCalcFNumBlock(opp_t *chip)
{
    ...
    int32_t lfo_pm = OPP_LFOApplyPMS(lfo & 127, pms);
    uint32_t kcode = OPP_CalcKCode(kcf, lfo_pm, (lfo & 0x80) != 0 && pms != 0 ? 0 : 1, dt);
    uint32_t fnum = OPP_KCToFNum(kcode);
...
static void OPP_PhaseCalcIncrement(opp_t *chip)
{
...
static void OPP_PhaseGenerate(opp_t *chip)
{
...
static void OPP_PhaseDebug(opp_t *chip)
{
    chip->pg_serial >>= 1;
    if (chip->cycles == 5)
    {
...
static void OPP_KeyOn1(opp_t *chip)
{
...
static void OPP_KeyOn2(opp_t *chip)
{
...
static void OPP_EnvelopePhase1(opp_t *chip)
{
...
static void OPP_EnvelopePhase2(opp_t *chip)
{
    uint32_t slot = chip->cycles;
...
static void OPP_EnvelopePhase3(opp_t *chip)
{
...
static void OPP_EnvelopePhase4(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 30) & 31;
    uint8_t inc = 0;
    ...
    /* OPP attack-rate slowdown: skip every Nth attack inc=1 to model the
     * OPP AR=18-30 plateau between OPM AR=12 and AR=13.  Zero skip = disabled. */
    if (inc > 0
        && chip->eg_state[slot] == eg_num_attack
        && chip->opp_atk_skip[slot] != 0)
    {
        chip->opp_atk_skip_cnt[slot]++;
        if (chip->opp_atk_skip_cnt[slot] >= chip->opp_atk_skip[slot])
        {
            chip->opp_atk_skip_cnt[slot] = 0;
            inc = 0;
        }
    }
    chip->eg_inc = inc;
...
static void OPP_EnvelopePhase5(opp_t *chip)
{
...
static void OPP_EnvelopePhase6(opp_t *chip)
{
...
static void OPP_EnvelopeClock(opp_t *chip)
{
    chip->eg_clock <<= 1;
...
static void OPP_EnvelopeTimer(opp_t *chip)
{
...
static void OPP_OperatorPhase1(opp_t *chip)
{
    uint32_t slot = chip->cycles;
...
static void OPP_OperatorPhase2(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 31) & 31;
    chip->op_phase = (chip->op_phase_in + chip->op_mod_in) & 1023;
}

static void OPP_OperatorPhase3(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 30) & 31;
...
static void OPP_OperatorPhase4(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 29) & 31;
    chip->op_logsin[1] = chip->op_logsin[0];
}

static void OPP_OperatorPhase5(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 28) & 31;
    chip->op_logsin[2] = chip->op_logsin[1];
}

static void OPP_OperatorPhase6(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 27) & 31;
...
static void OPP_OperatorPhase7(opp_t *chip)
{
...
static void OPP_OperatorPhase8(opp_t *chip)
{
...
static void OPP_OperatorPhase9(opp_t *chip)
{
...
static void OPP_OperatorPhase10(opp_t *chip)
{
...
static void OPP_OperatorPhase11(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 22) & 31;
    chip->op_out[2] = chip->op_out[1];
}

static void OPP_OperatorPhase12(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 21) & 31;
    chip->op_out[3] = chip->op_out[2];
}

static void OPP_OperatorPhase13(opp_t *chip)
{
...
static void OPP_OperatorPhase14(opp_t *chip)
{
...
static void OPP_OperatorPhase15(opp_t *chip)
{
...
static void OPP_OperatorPhase16(opp_t *chip)
{
...
static void OPP_OperatorCounter(opp_t *chip)
{
    if ((chip->cycles & 7) == 4)
    {
        chip->op_counter++;
...
static void OPP_Mixer2(opp_t *chip)
{
...
static void OPP_Output(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 27) & 31;
    chip->smp_so = (chip->mix_out_bit & 1) != 0;
    chip->smp_sh1 = (slot & 24) == 8 && !chip->ic;
    chip->smp_sh2 = (slot & 24) == 24 && !chip->ic;
}

static void OPP_DAC(opp_t *chip)
{
    int32_t exp, mant;
...
static void OPP_Mixer(opp_t *chip)
{
    uint32_t slot = (chip->cycles + 18) & 31;
    uint32_t channel = (slot & 7);
    // Right channel
...
static void OPP_Noise(opp_t *chip)
{
    ...
    if (noise_step)
    {
        if (!chip->ic)
        {
            ...
            uint8_t xr = ((chip->noise_lfsr >> 2) & 1) ^ chip->noise_bit;
            bit = rst | xr;
        }
    ...
    chip->noise_lfsr >>= 1;
    chip->noise_lfsr |= bit << 15;
}

static void OPP_NoiseTimer(opp_t *chip)
{
...
static void OPP_DoTimerA(opp_t *chip)
{
...
static void OPP_DoTimerA2(opp_t *chip)
{
    if (chip->cycles == 1)
    {
...
static void OPP_DoTimerB(opp_t *chip)
{
...
static void OPP_DoTimerB2(opp_t *chip)
{
...
static void OPP_DoTimerIRQ(opp_t *chip)
{
    chip->timer_irq = chip->timer_a_status || chip->timer_b_status;
}

static void OPP_DoLFOMult(opp_t *chip)
{
...
static void OPP_DoLFO1(opp_t *chip)
{
    uint16_t counter2 = chip->lfo_counter2;
    uint8_t of_old = chip->lfo_counter2_of;
    uint8_t lfo_bit, noise, sum, carry;
    uint8_t bb, sb, x, w2, w3, mulm, mb;
    uint8_t lfo_pm_sign;
    uint8_t ampm_sel = (chip->lfo_bit_counter & 8) != 0;
    ...
    w3 = !chip->ic && !chip->mode_test[1] && (!chip->lfo_clock_lock || chip->lfo_wave != 3) && (chip->lfo_val & 0x8000) != 0;
    ...
    carry = x || ((chip->cycles & 15) != 15 && chip->lfo_val_carry != 0 && chip->lfo_wave != 3);
    sum = carry + w2 + w3;
    lfo_bit = sum & 1;
    if (chip->lfo_wave == 3 && chip->lfo_clock_lock)
    {
        noise = chip->noise_lfsr & 1;
        lfo_bit |= noise;
    }
...
static void OPP_DoLFO2(opp_t *chip)
{
...
static void OPP_CSM(opp_t *chip)
{
...
static void OPP_NoiseChannel(opp_t *chip)
{
...
static void OPP_DoIO(opp_t *chip)
{
    // Busy
...
static void OPP_DoRegWrite(opp_t *chip)
{
    int32_t i;
...
static void OPP_DoIC(opp_t *chip)
{
...
static void OPP_TLRamp(opp_t *chip)
{
    uint32_t slot = chip->cycles;
    uint32_t channel = slot & 7;
    uint32_t step = ((chip->cycles + 1) & 31) < 8;
    uint32_t ramp = chip->ch_ramp_div[channel];

    uint32_t match = ramp == chip->opp_tl_cnt[channel];
    uint32_t tl = chip->sl_tl[slot];
    ...
    if (tl & 128)
    {
        if (match)
        {
            uint32_t val = chip->opp_tl[slot] >> 3;
            tl &= 127;
            if (val < tl)
                chip->opp_tl[slot]++;
            else if (val > tl)
        ...
        chip->opp_tl[slot] = tl << 3;
}

void OPP_Clock(opp_t *chip, int32_t *output, uint8_t *sh1, uint8_t *sh2, uint8_t *so)
{
    OPP_Output(chip);
    OPP_DAC(chip);
    OPP_Mixer2(chip);
    ...
    OPP_OperatorPhase16(chip);
    OPP_OperatorPhase15(chip);
    OPP_OperatorPhase14(chip);
    OPP_OperatorPhase13(chip);
    OPP_OperatorPhase12(chip);
    OPP_OperatorPhase11(chip);
    OPP_OperatorPhase10(chip);
    OPP_OperatorPhase9(chip);
    OPP_OperatorPhase8(chip);
    OPP_OperatorPhase7(chip);
    OPP_OperatorPhase6(chip);
    OPP_OperatorPhase5(chip);
    OPP_OperatorPhase4(chip);
    OPP_OperatorPhase3(chip);
    OPP_OperatorPhase2(chip);
    OPP_OperatorPhase1(chip);
    OPP_OperatorCounter(chip);

    OPP_EnvelopeTimer(chip);
    OPP_EnvelopePhase6(chip);
    OPP_EnvelopePhase5(chip);
    OPP_EnvelopePhase4(chip);
    OPP_EnvelopePhase3(chip);
    OPP_EnvelopePhase2(chip);
    OPP_EnvelopePhase1(chip);

    if (chip->opp)
        OPP_TLRamp(chip);

    OPP_PhaseDebug(chip);
    OPP_PhaseGenerate(chip);
    OPP_PhaseCalcIncrement(chip);
    OPP_PhaseCalcFNumBlock(chip);
...
void OPP_Write(opp_t *chip, uint32_t port, uint8_t data)
{
...
uint8_t OPP_Read(opp_t *chip, uint32_t port)
{
    uint16_t testdata;
    if ((port & 1) == 0)
        return 0xff;
...
uint8_t OPP_ReadIRQ(opp_t *chip)
{
    return chip->timer_irq;
}

uint8_t OPP_ReadCT1(opp_t *chip)
{
    if (chip->opp)
    {
        return chip->io_ct2;
...
uint8_t OPP_ReadCT2(opp_t *chip)
{
    if (chip->opp)
    {
        if (chip->mode_test[3])
...
void OPP_SetIC(opp_t *chip, uint8_t ic)
{
    if (chip->ic != ic)
    {
        chip->ic = ic;
...
void OPP_Reset(opp_t *chip, uint32_t flags)
{
    uint32_t i;
    memset(chip, 0, sizeof(opp_t));
    chip->opp = (flags & opp_flags_ym2164) != 0;
    OPP_SetIC(chip, 1);
    for (i = 0; i < 32 * 64; i++)
    {
        OPP_Clock(chip, NULL, NULL, NULL, NULL);
    }
    OPP_SetIC(chip, 0);
}

void OPP_SetAttackSkip(opp_t *chip, uint8_t slot, uint8_t skip)
{
    if (slot < 32)
    {
        chip->opp_atk_skip[slot] = skip;
    }
}
